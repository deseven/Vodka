...
switch ($page["name"]) {
	...
	case "utf8test":
		if (isset($_GET["test"])) {
			$vodka->replaceVar("RES","test passed!");
		}
		break;
	case "form/result":
		$vodka->replaceVar("GET",htmlspecialchars(var_export($_GET,true)));
		$vodka->replaceVar("POST",htmlspecialchars(var_export($_POST,true)));
}
